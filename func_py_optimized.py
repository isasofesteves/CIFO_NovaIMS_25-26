...
#Fitness Function
def population_fitness_rmse(population, target, rendered_cache=None):

    # ALTERADO: OPTIONAL RENDER
    """
    ...
    for start in range(0, len(population), 25):
        ...
        if rendered_cache is not None:
            # Reutiliza o render já feito 
            rendered_tensor = rendered_cache[start:end].to(device)
        else:
