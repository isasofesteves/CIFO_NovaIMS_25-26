...
def render(individual):
    ...
    """Renders the individual's triangles onto a blank canvas and returns the image as a numpy array."""
...
def mixed_mutation(indiv, mut_prob):
    
    """
    Randomly selects a mutation operator based on defined weights.
    
    Weights:
        - color_soft_mutation:         40%  (fine-tuning color)
        - creep_mutation:              30%  (local shape + color)
        - triangle_replacement_mutation: 20%  (global exploration)
        - color_hard_mutation:         10%  (escape local minima)
    """
...
def one_point_crossover(parent1, parent2, crossover_prob):
    ...
    """ Standard one-point crossover that splits the parents at a random point and swaps the tails """
