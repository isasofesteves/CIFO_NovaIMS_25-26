...
def render_triangles_cuda(output, triangles, bboxes, pop_size):
    ...
    tris = triangles[indiv_idx]
    bbox = bboxes[indiv_idx]

    for t in range(NUM_TRIANGLES-1, -1, -1):

        # bounding box test
        if cx < bbox[t, 0] or cx > bbox[t, 2] or cy < bbox[t, 1] or cy > bbox[t, 3]:
            continue
...
def compute_bboxes(population):
    ...
    bboxes[:, :, 0] = np.min(x, axis=2)
    bboxes[:, :, 1] = np.min(y, axis=2)
    bboxes[:, :, 2] = np.max(x, axis=2)
    bboxes[:, :, 3] = np.max(y, axis=2)
...
def render_population_cuda(population):
    ...
    # output buffer
    output_gpu = cuda.device_array((N, IMG_H, IMG_W, 4), dtype=np.float32)
